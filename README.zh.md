...
| **通知** | 设置 → 通知 | 回合结束/任务完结/审批等待/上下文将满 → **浏览器通知**（零配置，点一下授权）/ Bark / ntfy / webhook |
| **手机访问** | 设置 → 手机访问 | 局域网扫码配对，手机使用完整 DSH 界面（HMAC 设备 Cookie + 指数退避防爆破） |
| **文件变更** | 会话 ⋯ 菜单 | 双标签：**工作区变更**（git 基线 diff，纯只读）+ **操作记录**（AI 读写/bash 轨迹，含 ±行数与成败） |
...
│   └── ...
├── src/                   # 浏览器半源码（22 模块，构建拼接成 client.js）
├── scripts/
