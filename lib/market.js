...
export async function marketSearch(terms, sort) {
  ...
  const q = `${terms.trim()} dsh in:name`.trim()
  const cacheKey = `${q}::${mode}`
  const cached = marketCache.get(cacheKey)
  if (cached !== undefined && Date.now() - cached.at < MARKET_TTL_MS) {
    return { ...cached.payload, cached: true }
  }
  const url = new URL('https://api.github.com/search/repositories')
  url.searchParams.set('q', q)
  url.searchParams.set('sort', mode === 'updated' ? 'updated' : 'stars')
  ...
  const items = mode === 'default'
    ...
    : mode === 'name'
      ? [...mapped].sort(byName)
      : mapped
  const payload = { total: data.total_count ?? items.length, items, sort: mode, hasToken: token !== undefined && token !== '' }
