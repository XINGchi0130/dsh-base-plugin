...
import { MobileAuth, readCookie, COOKIE_NAME } from './auth.js'
import { servePwaAsset } from './pwa.js'

/** The upstream dsh web origin this proxy fronts (port overridable for tests). */
 ...
 */
export function startMobileProxy({ port, auth, onStateChange, version }) {
  return new Promise((resolve, reject) => {
    const server = httpRequestServer({ auth, onStateChange, version })
...
function httpRequestServer({ auth, onStateChange, version }) {
  const handle = (req, res) => {
    ...
    // GET auto-pair: /__dshm/auto?code=X&name=Y — cookie-less pairing for
    // headless testing (and a future in-URL pairing flow). Same rules as the
    // form POST (single-use code, backoff applies).
    if (req.method === 'GET' && req.url !== undefined && req.url.startsWith('/__dshm/auto')) {
      const params = new URL(req.url, 'http://x').searchParams
      const code = params.get('code') ?? ''
      const name = params.get('name') ?? 'auto'
      const result = auth.pair(code, name, ip)
      if (result === null) {
        res.writeHead(403, { 'content-type': 'text/plain; charset=utf-8', 'cache-control': 'no-store' })
        res.end('pairing failed (bad/expired code or backoff)')
        return
      }
      onStateChange()
      res.writeHead(302, {
