...
import { createUpstreamAuth } from './upstream-auth.js'
 ...
 * @param {() => Promise<import('node:buffer').Buffer | undefined>} [options.readUpstreamSecret]
 *   Resolves the upstream browser-session signing secret (see
 *   upstream-auth.js); when omitted no auth cookie is attached.
 * @returns {Promise<{ server: import('node:http').Server, port: number, close: () => Promise<void> }>}
 */
export function startMobileProxy({ port, upstreamPort, auth, onStateChange, version, readUpstreamSecret }) {
  const livePort = upstreamPortOf(upstreamPort)
  const upstreamAuth = readUpstreamSecret === undefined
    ? undefined
    : createUpstreamAuth(readUpstreamSecret, `${UPSTREAM_HOST}:${livePort}`)
  return new Promise((resolve, reject) => {
    const server = httpRequestServer({ auth, onStateChange, version, upstreamPort: livePort, upstreamAuth })
    ...
    const handleUpgrade = async (req, socket, head) => {
      ...
      // A cookie may need (re)minting before the hop — one await between the
      // pairing check and the upstream request. The socket already carries
      // its error/close fence from here on, so the async gap is safe.
      let upstreamCookie
      try {
        upstreamCookie = await upstreamAuth?.cookieHeader()
      } catch {
      ...
      const upgradeHeaders = {
        ...req.headers,
        host: `${UPSTREAM_HOST}:${livePort}`,
        'x-forwarded-for': ip,
      }
      delete upgradeHeaders.origin
      // dsh ≥ 0.1.2-alpha.1 requires a signed browser-session cookie on the
      // event channel too; replace the phone's pairing cookie (meaningless
      // upstream) with the minted session cookie.
      if (upstreamCookie !== undefined) upgradeHeaders.cookie = upstreamCookie
    ...
    // handleUpgrade is async (cookie minting): the sync try/catch below only
    // fences synchronous throws, so rejections get their own catch — an
    // unhandled one would kill the host.
...
function httpRequestServer({ auth, onStateChange, version, upstreamPort, upstreamAuth }) {
  const handle = (req, res) => {
    ...
    proxyRequest(req, res, headers, upstreamPort, upstreamAuth).catch(error => {
      journal(`PROXY-ERROR ${req.method} ${req.url}: ${String(error?.message ?? error).slice(0, 120)}`)
      try {
        if (!res.headersSent) {
          res.writeHead(500, { 'content-type': 'application/json; charset=utf-8' })
          res.end(JSON.stringify({ ok: false, error: 'mobile proxy hop failed' }))
        } else {
          res.destroy()
        }
...
async function proxyRequest(req, res, clientHeaders, upstreamPort, upstreamAuth) {
  const headers = {
  ...
  // dsh ≥ 0.1.2-alpha.1: attach the minted browser-session cookie so the
  // upstream's persistent browser authentication accepts the hop. During
  // the (rare) minting await the inbound body simply buffers — Node's
  // backpressure pauses the client until req.pipe starts below.
  try {
    const upstreamCookie = await upstreamAuth?.cookieHeader()
    if (upstreamCookie !== undefined) headers.cookie = upstreamCookie
  } catch {
    // Minting failed — forward unauthenticated (pre-0.1.2 behavior).
  }
  const upstream = httpRequest(
    {
      host: UPSTREAM_HOST,
    ...
    up => {
      // A 401 with our cookie attached means the signing secret rotated or
      // the record was reset — drop the cache so the next hop re-mints.
      if (up.statusCode === 401 && upstreamAuth !== undefined) {
        journal('upstream 401 despite session cookie — invalidating mint')
