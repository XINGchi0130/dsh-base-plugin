...
/** Backoff map bound: evict the earliest-expiring entry beyond this. */
const BACKOFF_MAX = 1024
...
export class MobileAuth {
  ...
  pair(code, deviceName, ip, now = Date.now()) {
    ...
    // 恒时比较（与 cookie HMAC 同标准）：双侧 SHA-256 摘要走
    // timingSafeEqual——8 字符码本身有 40 位熵 + 每 IP 退避兜底，
    // 这里补掉理论时序侧信道。
    const matches = live !== null && timingSafeEqual(
      createHash('sha256').update(live.code.toUpperCase()).digest(),
      createHash('sha256').update(code.toUpperCase()).digest(),
    )
    if (!matches) {
      ...
      // 有界：IPv6 前缀轮换可无限制造新键——超上限时逐出最早过期者
      //（旧条目本就快失效，逐出只轻微放宽该 IP 的退避）。
      if (this.backoff.size > BACKOFF_MAX) {
        let oldestKey = null
        let oldestUntil = Infinity
        for (const [key, value] of this.backoff) {
          if (value.until < oldestUntil) { oldestUntil = value.until; oldestKey = key }
        }
        if (oldestKey !== null) this.backoff.delete(oldestKey)
      }
      return null
    }
    live.used = true
 ...
 * decodeURIComponent 可对畸形值（如 `dshm=%`）抛 URIError——包住返回
 * undefined，让坏 cookie 落到 401 而非 500（fault fence 的噪音）。 */
