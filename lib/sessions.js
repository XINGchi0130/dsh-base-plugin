...
export async function listSessions(ctx) {
  const empty = {
    ...
    items: [],
    counts: { total: 0, live: 0, archived: 0, ghosts: 0, subagents: 0 },
  }
  ...
  for (const [id, header] of byId) {
    ...
    items.push({
      ...
      // 子代理会话标记（persistence header 的 origin/parentSession——
      // 审查/委派 fan-out 的子会话与用户主会话在 UI 分开展示）。
  ...
  return {
    ...
    counts: {
      ...
      subagents: items.reduce((n, item) => n + (item.subagent ? 1 : 0), 0),
    },
  }
}
 ...
 */
/**
 * Delete one session AND its subagent descendants (cascade). Children are
 * swept BEFORE the parent (leaves first); a live descendant (a subagent
 * still running) is skipped and reported instead of blocking the sweep.
 * @param {import('@deepseek-ai/cordis').Context} ctx - plugin context.
 * @param {string} rawId - the root session id to delete.
 * @returns the per-step outcome plus the child sweep summary.
 */
export async function deleteSession(ctx, rawId) {
  ...
  // Descendant collection (multi-level, BFS over header.parentSession):
  // 审查/委派 fan-out 的子会话与孙会话都挂在根会话之下。
  const byParent = new Map()
  for (const h of await persistence.list()) {
    const p = typeof h?.parentSession === 'string' ? String(h.parentSession) : ''
    if (p === '') continue
    if (!byParent.has(p)) byParent.set(p, [])
    byParent.get(p).push(String(h.id))
  }
  const descendants = []
  ...
  const sessionsSvc = ctx.get('sessions')
  const deletedSubagents = []
  const skippedSubagents = []
  // 子代理先删（叶先于父）；live 后代跳过并报告，不阻断整体。
  for (const child of descendants) {
    const isLive = sessionsSvc !== undefined && typeof sessionsSvc.get === 'function'
      && sessionsSvc.get(child) !== undefined
    if (isLive) {
      skippedSubagents.push({ id: child, reason: 'live' })
      continue
    }
    try {
      await deleteSessionInner(ctx, child)
      deletedSubagents.push(child)
    } catch (error) {
      skippedSubagents.push({ id: child, reason: error instanceof Error ? error.message : String(error) })
    }
  }
  const result = await deleteSessionInner(ctx, id)
  return Array.isArray(result) === false || typeof result !== 'object' || result === null
    ? result
    : { ...result, deletedSubagents, skippedSubagents }
}

async function deleteSessionInner(ctx, rawId) {
  const id = String(rawId ?? '')
  if (id === '') throw new Error('dsh-base-plugin: sessionId is required')
  ...
  let hadLog = false
  if (logDir !== null) {
    // TOCTOU 收口（P1）：上面的活性检查与本步 rm 之间存在窗口——用户
    // 在 announce 之后恰好重新打开该会话，宿主会重新物化日志并开始写
    // 入，rm 仍会删掉活跃会话的日志目录。进入唯一破坏步前再断言一次。
    const liveCheck = ctx.get('sessions')
    if (liveCheck !== undefined && typeof liveCheck.get === 'function' && liveCheck.get(id) !== undefined) {
      throw new Error(`dsh-base-plugin: session "${id}" became live again before deletion — close it first, then retry`)
    }
    hadLog = true
    let removed = false
