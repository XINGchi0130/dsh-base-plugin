...
export async function optimizePrompt(ctx, rawInput) {
  ...
  if (Buffer.byteLength(input, 'utf8') > 32 * 1024) throw new Error('dsh-base-plugin: 输入过长（上限 32KB）')
  ...
  let optimized = ''
  let failure = null
  // 分桶累积（StreamChunk 契约：block index 关联交错 delta——按到达
  // 顺序拼接曾在多文本块交错时产出乱序文本）；maxTokens 从源头限制
  // 复读循环（比 256KB 截断更早生效且不烧钱）。
  const order = []
  const texts = new Map()
  let received = 0
  try {
    ...
    for await (const chunk of chunks) {
      if (chunk.type === 'block-start') {
        if (chunk.blockType === 'text' && !texts.has(chunk.index)) {
          texts.set(chunk.index, '')
          order.push(chunk.index)
        }
      } else if (chunk.type === 'text-delta') {
        if (!texts.has(chunk.index)) {
          texts.set(chunk.index, '')
          order.push(chunk.index)
        }
        texts.set(chunk.index, texts.get(chunk.index) + chunk.text)
        // 输出上限：maxTokens 之外的兜底（异常循环的模型曾可无限累积）。
  ...
  optimized = order.map(i => texts.get(i)).join('')
  if (failure !== null) {
