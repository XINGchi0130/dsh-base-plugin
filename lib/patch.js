...
export function commit(state) {
  ...
  // 调用方现已 await commit（错误走 HTTP 400）；此兜底仅覆盖无人等待的
  // 拒绝（如 index.js 启动路径），不再完全静默。
  run.catch(error => {
  ...
  commitQueue.push(run)
  if (commitQueue.length > 50) commitQueue.splice(0, commitQueue.length - 50)
  return run
}
