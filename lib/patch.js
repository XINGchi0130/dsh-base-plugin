...
/**
 * 在同一互斥队列的临界区内完成"读盘 → 改字段 → 落盘"。
 *
 * 必要性：commit() 只串行化 commit→commit；persistNotify/mobile
 * toggle/persistMobile 这类同步直写若落在某次 install 的"handler 读
 * fresh → 分钟级 pnpm → 队列落盘"窗口内，其写入会被旧快照整体覆盖
 * （notify 配置/mobile 开关/刚配对设备静默丢失）。所有 state 写路径
 * 统一经此队列后，读-改-写不再交错。
 * @param {(state: object) => void} mutator - 就地修改读出的 state。
 * @returns {Promise<void>} 依次写入 state 文件与（若需要）patch 文件。
 */
export function mutateState(mutator) {
  ...
  run.catch(error => {
    console.warn(`dsh-base-plugin: state mutation failed: ${error instanceof Error ? error.message : String(error)}`)
  })
  commitTail = run.catch(() => {})
  return run
}
