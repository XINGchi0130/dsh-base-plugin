...
export async function exportMarkdown(ctx, sessionId) {
  ...
  const lines = []
  let docLen = 0
  // 单行软上限：DOC_MAX 检查在事件顶部，一次事件的若干字段仍可能把
  // 边界冲过数个 TOOL_TEXT_MAX——push 时按剩余预算截行，保证最终文档
  // 有界（≈ DOC_MAX + 最后一事件头部常量）。
  const push = (line) => {
    const budget = DOC_MAX + 65536 - docLen
    const text = typeof line === 'string' && line.length > budget ? `${line.slice(0, Math.max(0, budget))}\n…(文档大小上限截断)` : line
    lines.push(text)
    docLen += text.length + 1
  }
  push(`# ${title !== '' ? title : `会话 ${id.slice(0, 8)}`}`)
  lines.push('')
  ...
  for (;;) {
    ...
    for (const event of events) {
      ...
      seq = next
      if (docLen > DOC_MAX) { truncated = true; break }
      ...
      if (event.type === 'turn/start') {
        ...
        push(`### 🤖 助手${data.interrupted === true ? '（被中断）' : ''}`)
        push('')
        push(text !== '' ? text : '`(无文本输出)`')
        push('')
    ...
    if (truncated || stalled) break
  }

  if (truncated) {
    push('---')
    push('')
    push('> ⚠️ 文档达到大小上限，剩余内容未导出。')
  }
