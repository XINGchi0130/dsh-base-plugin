  ...
  return {
    ...
    async read(sessionId, terminalId, opKey) {
      ...
      const settled = await Promise.race([
        ...
        new Promise(resolve => { graceTimer = setTimeout(() => resolve(false), POLL_GRACE_MS) }),
      ])
      // done 先胜时清掉 grace 定时器（否则每次 read 滞留 1.2s 空定时器）
      ...
      let status = null
      try {
        // 仅当会话已在本进程活跃时查 status：走 agentFor 会经
        // agents.resume 把已关闭的会话从磁盘拉回内存——read 轮询曾把
        // 每个关闭的终端会话永久钉活。
        const agents = ctx.get('agents')
        const live = agents !== undefined && typeof agents.get === 'function' ? agents.get(sessionId) : undefined
        if (live !== undefined) {
          const list = terminals().list(live)
