...
import { fileOpsRoutes, gitRoutes, monitorRoutes, promptOptimizerRoutes, terminalsRoutes } from './routes/panels.js'
...
export function registerRoutes(ctx, mobileControls) {
  ...
  const groups = [
    ...
    monitorRoutes(ctx),
    fileOpsRoutes(ctx),
    promptOptimizerRoutes(ctx, deps),
  ...
  const handled = async (req, res) => {
    ...
    // Origin 头存在且跨源 → 403（GET 含在内——/git/status 有副作用、
    // /notify 与 /export/markdown 泄露凭据与转写，不能再豁免）。同源
    // fetch 与 curl（无 Origin）放行。
    if (!sameOrigin(req)) {
      sendJson(res, 403, { ok: false, error: 'cross-origin request rejected' })
      return true
    }
    // Host 白名单：DNS rebinding 下 Origin 与 Host 同为攻击者域名、同源
    // 检查形同虚设——只认本机回环/localhost 的 Host（移动代理经
    // 127.0.0.1 上游转发，同样携带回环 Host）。
    const host = String(req.headers.host ?? '')
    if (!/^127\.0\.0\.1(:\d+)?$/.test(host) && !/^localhost(:\d+)?$/i.test(host) && !/^\[::1\](:\d+)?$/.test(host)) {
      sendJson(res, 403, { ok: false, error: 'unrecognized host' })
      return true
    }
