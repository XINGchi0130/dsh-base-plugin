...
export async function scanUsage(ctx, { forceRescan = false, startDay = '', endDay = '', granularity = 'auto' } = {}) {
  ...
  for (const record of records) {
    ...
    for (const event of events) {
      ...
      // seq 防御（其他三个折叠器同款）：非数字 seq 直接忽略（写入会使
      // lastSeq 变 undefined → 落盘丢字段 → validEntry 判死 → 下轮从 0
      // 全量重折，append-only 账本永久翻倍）；游标只升不降（回退同样
      // 触发重折重复记账）。
      if (typeof event.seq === 'number' && event.seq > (entry.lastSeq ?? -1)) {
