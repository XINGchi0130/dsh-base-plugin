...
async function mcpHealthImpl(ctx) {
  ...
  // 失联自剪除（P2）：从 inventory 消失但未触发外部 drop 的会话，其
  // cursors/pending 条目会永久残留（数字只增不减的慢性泄漏）——就地对
  // 照 headers 清掉；epoch 递增使在飞折叠的写入栅栏失效。
  {
    const liveIds = new Set(headers.map(h => String(h?.id ?? '')).filter(x => x !== ''))
    let pruned = false
    for (const id of [...cursors.keys()]) {
      if (!liveIds.has(id)) { cursors.delete(id); pruned = true }
    }
    if (pruned) epoch += 1
  }
  for (const header of headers) {
    ...
    // 时限淘汰（P2）：调用方崩溃/结果丢失的 pending 条目按 1 小时上限
    // 丢弃（无界 Map 的慢性泄漏）；正常配对在毫秒级完成，不受影响。
    const staleBefore = Date.now() - 60 * 60 * 1000
    for (const [callId, call] of pending) {
      if (typeof call.at === 'number' ? call.at < staleBefore : true) pending.delete(callId)
    }
    for (;;) {
      let page
      try {
      ...
      // 停滞栅栏（export.js 同款）：seq 契约违反（回退/重复/缺失）时游标
      // 不前进——不 break 就是无限 await，且 healthInflight 单飞永不释放。
      // 游标只升不降：聚合（calls/latency）绝不因回退事件重复累加。
      const before = seq
      ...
      if (seq <= before) break // seq 契约违反：停滞即跳出（见上）
      state.lastSeq = seq
    }
  }
