...
/** Record + deliver. Browser channel only records (the client polls and
 * fires Notification API); the other channels POST as before. */
export async function sendNotification(fetcher, n, title, body) {
  EVENT_LOG.push({ at: Date.now(), title, body })
  if (EVENT_LOG.length > EVENT_LOG_MAX) EVENT_LOG.splice(0, EVENT_LOG.length - EVENT_LOG_MAX)
  if (n.channel === 'browser') return true
  try {
...
/** Drain notify events past a cursor (browser channel polling). */
export function notifyEvents(sinceMs) {
  const since = Number(sinceMs) || 0
  return EVENT_LOG.filter(e => e.at > since)
}
