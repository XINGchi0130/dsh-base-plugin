...
export function emptyNotifySection() {
  return { enabled: false, channel: 'webhook', url: '', barkKey: '', ntfyTopic: '', turnEnd: true, jobs: true, approvals: true, context: true, quietUntil: 0 }
}
...
export async function sendNotification(fetcher, n, title, body) {
  try {
    ...
    try {
      ...
      // 消费（小响应）或取消 body：不读不取消会让连接滞留到 GC——
      // 通知高频时耗尽连接池。
      try {
