...
export async function fileOps(ctx, sessionId) {
    ...
    for (const event of events) {
      ...
      if (event.type === 'tool/call') {
        ...
        if (diffs.length === 0) {
          ...
          ops.unshift({ opId: `op${event.seq}`, time: call.time, turn: call.turn, tool: call.tool, added: 0, deleted: 0, diff: null, failed })
  ...
  const files = [...fold.files.entries()]
    .map(([path, ops]) => ({
      ...
      ops: ops.slice(0, OPS_PER_FILE).map(({ opId, time, turn, tool, added, deleted, failed }) => (
        { opId, time, turn, tool, added, deleted, failed }
      )),
