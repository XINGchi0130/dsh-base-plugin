 ...
 * (fileCounts / fileTotals never shrink). Visible rows are windowed:
 * trimFold caps total rows, and the payload keeps the newest TOUCH_LIMIT
 * file keys (beyond-window keys drop WITH their rows — their lifetime
 * counts/totals live on in the side Maps, unseen until touched again).
 *
...
async function fileOpsImpl(ctx, id) {
  ...
  for (;;) {
    ...
    // 停滞栅栏（export.js 同款）：后端违反 seq 单调契约（回退/重复/缺
    // 失）时游标可能不前进——不 break 就是无限 await 循环，且单飞锁
    // opsInflight 永不释放，此后 /api/fileops 对所有调用方永久挂死。
    // 游标只升不降：seq 回退的事件照常入列（显示无害），重复事件至多
    // 多显示一次，不会翻倍累积。
    const before = fold.lastSeq
    for (const event of events) {
      const next = (event.seq ?? fold.lastSeq) + 1
      if (next > fold.lastSeq) fold.lastSeq = next
      ...
      if (fold.lastSeq <= before) break // seq 契约违反：停滞即跳出（见上）
    }
  }
