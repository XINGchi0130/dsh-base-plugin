...
/** Per-session in-flight folds (declared beside its consumer: a late
 * `const` referenced by an earlier function is a TDZ landmine — the exact
 * shape of the patch.js incident). */
const foldInflight = new Map()

/** Fold with per-session single-flight: a poll racing a manual refresh
 * reuses the SAME in-flight fold instead of double-counting the same events
 * into one shared entry (the shared-entry mutation made the race corrupt
 * totals silently). */
function foldTokens(ctx, id) {
  const existing = foldInflight.get(id)
  if (existing !== undefined) return existing
  const run = foldTokensImpl(ctx, id).finally(() => { foldInflight.delete(id) })
  foldInflight.set(id, run)
  return run
}
 ...
 */
async function foldTokensImpl(ctx, id) {
  const persistence = ctx.get('sessionPersistence')
  let entry = cursors.get(id)
  if (entry === undefined) {
    entry = { lastSeq: -1, tokens: { ...ZERO_TOKENS }, requests: 0, firstTime: 0, lastTime: 0 }
  }
  try {
    // 游标哨兵 -1（0 是合法 seq 值：首事件 seq=0 的会话在旧哨兵下每轮
    // 从 0 重读，token 永久重复累计）。
    const from = entry.lastSeq < 0 ? 0 : entry.lastSeq + 1
    const page = await persistence.readFrom(id, from)
    for (const event of page?.events ?? []) {
      // monotonic 栅栏（与 mcp-health 同款）：畸形/回退的 seq 不推进
      // 游标、不累计，防止重复计数与 readFrom(id, NaN)。
      if (typeof event.seq !== 'number' || !Number.isFinite(event.seq) || event.seq <= entry.lastSeq) continue
      ...
      if (typeof event.time === 'number') {
        if (entry.firstTime === 0) entry.firstTime = event.time
...
/** The projection values block for a cold session, via the projection cache. */
function cachedValues(ctx, header) {
  try {
    const cache = ctx.get('sessionProjectionCache')
    if (cache === undefined || typeof cache.cachedSnapshot !== 'function') return null
    return cache.cachedSnapshot(header)?.values ?? null
  } catch {
    return null
  }
}
...
export async function sessionMonitor(ctx, rawId) {
  ...
  // ── official sessionStats + contextPressure projections ───────────────
  let live = false
  let stats = null
  let context = null
  let breakdown = null
  ...
  try {
    if (liveSession !== undefined) {
      ...
      if (header !== undefined) {
        const values = cachedValues(ctx, header)
        if (values !== null) {
          stats = values.sessionStats ?? null
          context = values.contextPressure ?? null
          breakdown = values.contextBreakdown ?? null
    ...
    try {
      ...
      for (const e of entries) {
        ...
        try {
          ...
          if (header !== undefined) {
            const childStats = cachedValues(ctx, header)?.sessionStats ?? null
            if (childStats !== null) {
        ...
        subagents.push({
          ...
          firstTime: mini.firstTime,
          lastTime: mini.lastTime,
        })
      }
    } catch (error) {
