...
import { fileOps, fileOpsAvailable, fileOpsDiff } from '../file-ops.js'
...
export function fileOpsRoutes(ctx) {
  return [
    ...
    // GET /dsh-base-plugin/api/fileops/diff?sessionId=...&opId=... —— 单条
    // 操作的完整 diff 按需取（轮询载荷只带摘要；被逐出缓存的返回
    // evicted 标记，前端提示不可展开）。
    {
      method: 'GET',
      path: '/dsh-base-plugin/api/fileops/diff',
      handler: async (_req, res, url) => {
        const value = fileOpsDiff(url.searchParams.get('sessionId') ?? '', url.searchParams.get('opId') ?? '')
        sendJson(res, 200, { ok: true, value })
      },
    },
    // GET /dsh-base-plugin/api/fileops?sessionId=... —— 该会话经 write/edit
    // 工具的文件操作时间线（按文件分组、最新在前；diff 来自官方
    // tool/result 的 meta.diffs）。
    {
      method: 'GET',
