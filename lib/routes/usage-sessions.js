  ...
  return [
    ...
    // 从 seq 0 全量重扫；start/end 收窄 token 表与趋势序列的范围。
    {
      ...
      handler: async (req, res, url) => {
        // force=1 是带副作用的 GET（全量重扫 + 占 busy 单飞锁）：strict
        // 同源拒绝无 Origin 的 no-cors 盲请求（防反复打 force 长期阻塞
        // 安装等长操作）。普通增量读取不强制。
        if (url.searchParams.get('force') === '1' && !sameOrigin(req, true)) {
...
export function sessionsRoutes(ctx) {
  return [
    ...
    // GET /dsh-base-plugin/api/sessions/search?q=... —— 跨会话全文搜索
    // （用户/助手消息文本；空格分词全 AND；有界扫描，见 lib/search.js）。
    // strict 同源：查询本身无副作用，但结果含会话内容片段。
    {
      method: 'GET',
      path: '/dsh-base-plugin/api/sessions/search',
      handler: async (req, res, url) => {
        if (!sameOrigin(req, true)) {
          sendJson(res, 403, { ok: false, error: 'cross-origin request rejected' })
          return
        }
        const value = await searchSessions(ctx, url.searchParams.get('q') ?? '', listSessions)
        sendJson(res, 200, { ok: true, value })
      },
    },
