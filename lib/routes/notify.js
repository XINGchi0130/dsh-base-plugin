 ...
 * 经 mutateState 的互斥临界区：与 install 的分钟级 commit 窗口并发时，
 * 同步直写会被旧快照覆盖（notify 配置静默丢失）。 */
  ...
  return [
    ...
    // POST /dsh-base-plugin/api/notify/test —— 用当前保存的配置发一条测试。
    {
      ...
      handler: async (_req, res) => {
        ...
        sendJson(res, 200, { ok: true, value: { sent: true, channel: n.channel } })
      },
    },
